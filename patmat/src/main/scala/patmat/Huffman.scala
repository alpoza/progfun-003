...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def aux(sub: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = sub match {
      case Nil => acc
      case x :: xs => aux(xs.filter(_ != x), acc :+ (x, 1 + xs.count(_ == x)))
    }
    aux(chars, List())
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    def makeLeafList(freqs: List[(Char, Int)]): List[CodeTree] = {
      def aux(acc: List[CodeTree], sub: List[(Char, Int)]): List[CodeTree] = sub match {
        case Nil => acc
        case x :: xs => aux(acc :+ Leaf(x._1, x._2), xs)
      }
      aux(List(), freqs.sortWith((a, b) => a._2 < b._2))
    }
    until(singleton, combine)(makeLeafList(times(chars))).head
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def aux(ts: CodeTree, bs: List[Bit], accu: List[Char]): List[Char] = bs match {
      case Nil => ts match {
        case l: Leaf => accu :+ l.char
        case f: Fork => println("Invalid encoding!"); accu
      }
      case x :: xs => ts match {
        case l: Leaf => aux(tree, bs, accu :+ l.char)
        case f: Fork => aux(if (x == 0) f.left else f.right, xs, accu)
      }
    }
    aux(tree, bits, List())
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    case Nil =>
      println("Can't encode character " + char); List()
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def aux(table: CodeTable, chars: List[Char], accu: List[Bit]): List[Bit] = chars match {
      case Nil => accu
      case x :: xs => aux(table, xs, accu ::: codeBits(table)(x))
    }
    aux(convert(tree), text, List())
  }
}
