...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def maxRet(t1: Tweet, t2: Tweet): Tweet = {
...
object GoogleVsApple {
  ...
  def tweetsFrom(brand: List[String]): TweetSet =
    TweetReader.allTweets.filter(t => brand.exists(s => t.text.contains(s)))
  
  lazy val googleTweets: TweetSet = tweetsFrom(google)
  lazy val appleTweets: TweetSet = tweetsFrom(apple)
